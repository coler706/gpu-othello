 ...
 * Gonzalez.
 **/
public class OthelloDisplay extends JFrame implements OthelloObserver, OthelloPlayer, ActionListener {
  ...
  protected void handleMove(Move m) {
    ...
    board.move(m, turn);
    // 10-ms update?
    boardComponent.setTurn(turn.opposite());
  ...
  public void OnMove(Move m, long blackTimeout, long whiteTimeout) {
    ...
    if (turn == OthelloSide.BLACK)
      stateLabel
          .setText(blackName + "'s turn.  Time left: " + OthelloUtil.showTime(blackTimeout) + "  Other move: " + m);
    else
      stateLabel
          .setText(whiteName + "'s turn.  Time left: " + OthelloUtil.showTime(whiteTimeout) + "  Other move: " + m);
  }
...
class BoardComponent extends JPanel {
  ...
  public void setTurn(OthelloSide turn) {
    this.turn = turn;
  }
  ...
  public void paintComponent(Graphics graphics) {
    ...
    for (int i = 0; i < 8; i++) {
      for (int j = 0; j < 8; j++) {
        ...
        if (board.checkMove(new Move(i, j), turn)) {
          float interpolate = 0.0f;
          g.setColor(
              turn == OthelloSide.BLACK ? new Color(0, Math.max(0, Math.min(255, (int) (153.0f * interpolate))), 0)
                  : new Color((int) Math.max(0, Math.min((int) (255.0f * (1.0f - interpolate)), 255)),
                      (int) Math.max(0, Math.min((int) (153.0f * interpolate + 255.0f * (1.0f - interpolate)), 255)),
                      (int) Math.max(0, Math.min(255, 255.0f * (1.0f - interpolate)))));
          g.drawOval(x + 2, y + 2, squareSize - 4, squareSize - 4);
        }
      }
    }
  }
